...
fn switch_duplicate_default_is_a_syntax_error() {
    // ES §14.12: at most one default clause. oxc reports it as a parse
    // diagnostic, which the driver surfaces before lowering (a second
    // default would otherwise bind the shared `default_entry` label twice
    // and panic in `FunctionBuilder::bind`).
