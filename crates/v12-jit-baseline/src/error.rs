//! Error types and limits for the baseline JIT.

use v12_bytecode::Opcode;

/// Maximum number of bytecode instructions a function may contain before the
/// baseline JIT refuses to compile it.
///
...
/// Reasons a function could not be baseline-compiled.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum JitError {
    /// The `jit` cargo feature is disabled.
    Disabled,
    /// The function's bytecode is too large for the baseline tier.
    TooLarge {
    ...
    /// The function uses an opcode the baseline tier does not yet support.
    UnsupportedOpcode(Opcode),
    /// The function uses a wide operation the baseline tier does not yet support.
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::TooLarge { len, limit } => {
                write!(f, "function too large for baseline JIT: {len} > {limit}")
            }
            Self::UnsupportedOpcode(op) => {
                write!(f, "unsupported opcode for baseline JIT: {op:?}")
            }
            Self::UnsupportedWideOp(msg) => {
                write!(f, "unsupported wide op for baseline JIT: {msg}")
            }
