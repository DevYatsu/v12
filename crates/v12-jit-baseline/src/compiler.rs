...
fn build_and_verify_ir(bytecode: &FunctionBytecode) -> Result<Vec<PcMapEntry>, JitError> {
    ...
    while pc < bytecode.instrs.len() {
        ...
        // Normal opcodes. The twelve binary-operator arms (arithmetic and
        // comparison) share one emission shape — call the two-operand helper,
        // store the result, continue — so they are generated by `binop!`
        ...
        match op {
            ...
            Opcode::Add => binop!(fn_add),
            Opcode::Sub => binop!(fn_sub),
            Opcode::Mul => binop!(fn_mul),
            Opcode::Div => binop!(fn_div),
            Opcode::Eq => binop!(fn_eq),
            Opcode::Ne => binop!(fn_ne),
            Opcode::Lt => binop!(fn_lt),
            Opcode::Le => binop!(fn_le),
            Opcode::Gt => binop!(fn_gt),
            Opcode::Ge => binop!(fn_ge),
            Opcode::StrictEq => binop!(fn_strict_eq),
            Opcode::StrictNe => binop!(fn_strict_ne),
            Opcode::Neg => {
                let dst = instr.a() as usize;
                let src = instr.b() as usize;
...
/// Emits the shared binary-operator shape: call the two-operand helper with
/// `vars[lhs]`, `vars[rhs]`, store the result in `vars[dst]`, and continue at
/// the next instruction's block.
...
fn emit_binop(
    ...
    ensure_regs(&[dst, lhs, rhs], max_regs)?;
    let a = builder.use_var(vars[lhs]);
    let b = builder.use_var(vars[rhs]);
    let call = builder.ins().call(helper, &[a, b]);
    let res = builder.inst_results(call)[0];
    builder.def_var(vars[dst], res);
    jump_to_next(builder, blocks, pc + 1, exit_block);
    Ok(())
}
...
fn make_exec_closure(bytecode: &FunctionBytecode) -> JitExecFn {
    ...
    Box::new(move |regs: &mut [JsValue]| {
        ...
        loop {
            ...
            // The twelve binary-operator arms share one execution shape —
            // call the two-operand helper on the operand bits, store the
            // result — generated by `binop!` rather than written out twelve
            // times.
            macro_rules! binop {
                ($helper:expr) => {{
                    let (dst, a, b) =
            ...
            match op {
                ...
                Opcode::Add => binop!(runtime::jit_add),
                Opcode::Sub => binop!(runtime::jit_sub),
                Opcode::Mul => binop!(runtime::jit_mul),
                Opcode::Div => binop!(runtime::jit_div),
                Opcode::Neg => {
                    let dst = instr.a() as usize;
                    let src = instr.b() as usize;
                ...
                Opcode::Eq => binop!(runtime::jit_eq),
                Opcode::Ne => binop!(runtime::jit_ne),
                Opcode::Lt => binop!(runtime::jit_lt),
                Opcode::Le => binop!(runtime::jit_le),
                Opcode::Gt => binop!(runtime::jit_gt),
                Opcode::Ge => binop!(runtime::jit_ge),
                Opcode::StrictEq => binop!(runtime::jit_strict_eq),
                Opcode::StrictNe => binop!(runtime::jit_strict_ne),
                Opcode::Jump => {
