...
- **cargo nextest** — **563 passed, 1 skipped** (`cargo nextest run --workspace`, 6.0 s). Covers `v12-bytecode` decode sweeps (6/6), `v12-bccompiler` (132), `v12-interp`, `v12-engine` builtins, `v12-jit-*`, `v12-cli` spawns.
...
- **Object/Function/Array protos** — Step 3b: `Object.keys/values/entries/hasOwnProperty`, `Function.prototype.call/apply/bind/toString`, `Array.slice/sort`, `hasOwnProperty/valueOf/toString` on all objects → `callee` 3 690 → 3 547 (−143); language +86 pass.
- **For-of destructuring** — Step 4: `crates/v12-bccompiler/src/stmt.rs` now handles array/object destructuring and member-expression targets in `for-of` via temp + recursive `assign_for_of_value` (defaults, rest via `CopyArrayRest`/`CopyObjectRest`, `member_parts` reuse) → `for-of` 479 → ~389 (−90); language +94 pass (363/752 for-of).
- **Private field** — `#x` class key via `static_key_text` (`#` prefix). Still largely unsupported (~1 349 fails in expressions) — top remaining remains `callee is not a function` (3 547).
