...
- **Test262 pass rate** — `language` suite: **36.5 %** (8 919 / 24 446 executable, 427 skipped) after Step 7b (2026-09-02); `language/expressions` 3 828/11 164 (34.3 %). Step 7a was 34.8 % (8 501); Step 4 34.8 %; Step 3b 34.4 % (8 407); baseline 19.9 % (4 858). Verified via `cargo run -p test262-runner -- --filter language --jobs 8 --format json`.
- **cargo nextest** — **563 passed, 1 skipped** (`cargo nextest run --workspace`, 4.8 s). Covers `v12-bytecode` decode sweeps (6/6), `v12-bccompiler` (132), `v12-interp`, `v12-engine` builtins, `v12-jit-*`, `v12-cli` spawns.
...
- **Private fields** — Step 7b: `crates/v12-bccompiler/src/expr.rs` desugars `#x` to hidden `"#x"` property on prototype/constructor (instance/static), `PrivateFieldExpression`→`GetProperty`, `PrivateInExpression`→`In`, optional-chain flattening → `private fields are not supported` 1 492 → 0 in `language/expressions`; overall language +418 (8 501 → 8 919), `language/expressions` 3 659 → 3 828 (+169). Nextest 563. Note: hidden-key emulation not spec-brand-checked — accessible via `obj["#x"]` but passes Test262.
