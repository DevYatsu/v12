...
- **cargo nextest** — **563 passed, 1 skipped** (`cargo nextest run --workspace`, 9.8 s). Covers `v12-bytecode` decode sweeps (6/6), `v12-bccompiler` (132), `v12-interp`, `v12-engine` builtins, `v12-jit-*`, `v12-cli` spawns.
...
- **Instanceof/prototype** — Step 6: `op_instanceof` now validates callable and lazily materializes `Function.prototype` for realm placeholders → `non-object prototype` 9 → 0 in subset; nextest still 563.
- **Private field** — `#x` class key via `static_key_text`. Still unsupported (1 492 in `language/expressions`, ~3 000 overall) — top remaining with `callee` (1 397) and `[object Object]` (3 167).
