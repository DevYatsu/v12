...
- **Test262 pass rate** — `language` suite: **~34.8 %** (8 501 / 24 446 executable, 427 skipped) after Step 4 (2026-09-02). Step 3b was 34.4 % (8 407); Step 3a 34.0 % (8 321); Step 2 33.6 % (8 216); Step 1 33.0 % (8 066); baseline 19.9 % (4 858). Verified via `cargo run -p test262-runner -- --filter language --jobs 8 --format json`.
- **cargo nextest** — **563 passed, 1 skipped** (`cargo nextest run --workspace`, 6.5 s). Covers `v12-bytecode` decode sweeps (6/6), `v12-bccompiler` (132), `v12-interp`, `v12-engine` builtins, `v12-jit-*`, `v12-cli` spawns.
...
- **Dynamic import** — `import(source)` desugared to `Closure #NATIVE_IMPORT_INDEX` call (254). Step 5 registered `ModuleImport` native stub → `ModuleImport is not registered` 306 → 324 `dynamic import not supported` (same count, now proper TypeError instead of registry panic).
- **Collector walk** — `crates/v12-bccompiler/src/collect.rs` now walks `Switch`/`With`/`New`/`Template`/`Yield`/`Await` and destructuring assignment targets. Closed `nested function missing from plans` 684 → 62 (−622) after Step 5 catch-destructuring fix.
...
- **Object/Function/Array protos** — Step 3b: `Object.keys/values/entries/hasOwnProperty`, `Function.prototype.call/apply/bind/toString`, `Array.slice/sort`, `hasOwnProperty/valueOf/toString` on all objects → `callee` 3 690 → 3 547 (−143).
- **For-of destructuring** — Step 4: `stmt.rs` handles array/object destructuring and member targets via `assign_for_of_value` (defaults, rest) → `for-of` 479 → 389 (−90); language +94.
- **Minor syntax** — Step 5: `BigInt` literals → `BigInt` heap values, `tagged template` → desugared call, `with` → best-effort, `catch` destructuring via `lower_binding_pattern`, sentinel `0xFFFFFFFF` → `not a function` → closed BigInt 147 → 0, tagged 43 → 0, with 108 → 0; language +~60.
- **Private field** — `#x` class key via `static_key_text` (`#` prefix). Still largely unsupported (1 492 in `language/expressions` alone, ~3 000 overall) — top remaining with `callee` (1 395) and `[object Object]` (3 138).
