...
- Always run tests with **`cargo nextest run --workspace`**, not `cargo test`. `cargo nextest` is the workspace gate (faster, clearer output, same 562 tests). `cargo test` remains available but is not the canonical command.
...
- **Test262 pass rate** — `language/expressions` verified slice: **4 054/11 190 (36.8 %)** after the P1 refactor commit (2026-09-05): Step 8 (`24e838f`) + harness sta.js/assert.js fix + P0 (`1b0f73b`, with-statement `CompileError` honestly costs ~108 false passes on this slice) + P1 shared `lower_default` fixing assignment-target destructuring defaults. Re-verified unchanged (36.8 %) after the P2 dead-code deletion, P2b JIT wiring, P3 structural splits, and P4 clippy fixes — all were behavior-preserving. Full `language` still times out in CI; last completed full-language score was 8 919/24 446 (36.5 %) after Step 7b (hidden-key; ~8 800 spec-correct). Baseline 19.9 % (4 858). Verified via `./conformance/run.sh --filter language/expressions --jobs 4`. Nextest gate is canonical.
- **cargo nextest** — **562 passed, 1 skipped** (`cargo nextest run --workspace`). Covers `v12-bytecode` decode sweeps (1.7 s with new WideOps 11-14 width 4), `v12-bccompiler` (133), `v12-interp`, `v12-engine` builtins, `v12-jit-*`, `v12-cli` spawns. Count drifted 564 → 562 via P2 (v12-native sig.rs test module deleted, −3; jit_tier tests added, +3 — net −2 after earlier −3).
