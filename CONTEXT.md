...
- **Test262 pass rate** — `language` suite: **~34.8 %** (8 501 / 24 446 executable, 427 skipped) after Step 4; `language/expressions` subset 3 659/11 164 (32.8 %) after Step 6 triage (2026-09-02). Step 3b was 34.4 % (8 407); Step 3a 34.0 % (8 321); Step 2 33.6 % (8 216); Step 1 33.0 % (8 066); baseline 19.9 % (4 858). Verified via `cargo run -p test262-runner -- --filter language --jobs 8 --format json`.
- **cargo nextest** — **563 passed, 1 skipped** (`cargo nextest run --workspace`, 4.9 s). Covers `v12-bytecode` decode sweeps (6/6), `v12-bccompiler` (132), `v12-interp`, `v12-engine` builtins, `v12-jit-*`, `v12-cli` spawns.
...
- **Object Object triage** — Step 7a: `engine.rs`/`interp` `to_display_string` now renders plain-object `Test262Error` via `message`/`name` shape lookup → opaque `threw: [object Object]` 3 167 → 0 (reclassified to `Expected a undefined to be thrown` 975, `abrupt completion` 205, `SameValue` mismatches, etc.); pass unchanged 3 659/11 164 but bucket now actionable. Nextest 563.
- **Private field** — `#x` class key via `static_key_text`. Still unsupported (1 492 in `language/expressions`, ~3 000 overall) — now the single largest remaining bucket after triage, tied with `callee` (1 397).
