...
- **cargo nextest** — **563 passed, 1 skipped** (`cargo nextest run --workspace`). Covers `v12-bytecode` decode sweeps (1.7 s with new WideOps 11-14 width 4), `v12-bccompiler` (133), `v12-interp`, `v12-engine` builtins, `v12-jit-*`, `v12-cli` spawns. Count drifted 564 → 563 via P2 deletions (−3), jit_tier tests (+3), re-enabled async gate test (+1 — the "skip" is an unrelated spawn test).
