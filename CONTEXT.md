...
- **Test262 pass rate** — `language/expressions` verified slice: **4 047/11 190 (36.8 %)** after Step 8 (`24e838f`) + the harness sta.js/assert.js always-prepend fix (2026-09-02). Full `language` still times out in CI; last completed full-language score was 8 919/24 446 (36.5 %) after Step 7b (hidden-key; ~8 800 spec-correct). Baseline 19.9 % (4 858). Verified via `cargo run -p test262-runner -- --filter language/expressions --jobs 4 --format json` — note `--json-out` is only written when `--format` includes tap/human; capture stdout for pure json. Nextest gate is canonical.
