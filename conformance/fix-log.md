...
- **Before (base `567eef7`):** `built-ins/Array` **1 085 pass / 1 915 fail / 82 skip (36.2 %)**; `built-ins/String` **679 pass / 544 fail (55.5 %)**.
- **After:** `built-ins/Array` **1 226 / 1 774 / 82 (40.9 %)** — **+141**, zero regressions (TAP pass-set diff: 0 tests that passed at base now fail). `built-ins/String` **731 / 492 (59.8 %)** — **+52**, zero regressions. `built-ins/ArrayIteratorPrototype` unchanged at 9. Panics: 0 in both logs.
- **Root cause:** engine-allocated arrays were built with `JsObject::array` and never linked to `%Array.prototype%`. Literal arrays are linked by the interpreter's `NewArray` arm (`crates/v12-interp/src/execute.rs`), but the engine allocators bypassed it, and `crates/v12-heap/src/object.rs` cannot self-link (no realm access). Symptom: `The value of __split.constructor is expected to equal the value of Array`, and `arr.foo = 42` read back the length because the object kept the empty-object root shape while `properties[0]` already held the length Smi, so the first added property took slot 0.
- **Fix:** added `Ctx::alloc_array` / `Ctx::alloc_array_sparse` / `Ctx::finish_array` (`crates/v12-engine/src/builtins/ctx.rs`). Each roots the fresh object, stamps `%Array.prototype%` resolved via `ctx.intrinsic("Array")` then its `prototype` (the same idiom as `regexp::link_regexp_proto`), and binds the canonical `root --length--> child` array shape with spec `length` attrs (`writable`, non-enumerable, non-configurable). Allocation-time only; no per-element work. Routed `array.rs`'s single `new_array` helper (used by `slice`, `splice`, `flat`, `concat`, `of`, `from`) and `array_construct`'s sparse + dense paths through it, and `string.rs`'s `split`/`match`/`matchAll` array allocations (`array_of_strings`, `match_text_to_array`, `string_match_all`) through `alloc_array`. Literal arrays and `v12-heap` untouched.
- **Files:** `crates/v12-engine/src/builtins/ctx.rs`, `crates/v12-engine/src/builtins/array.rs`, `crates/v12-engine/src/builtins/string.rs`.
- **Verification:** `cargo build --workspace` clean; `cargo nextest run --workspace` **640 passed / 0 skipped / 0 failed**; `cargo clippy --workspace --all-targets` **0 errors**; `cargo fmt -p v12-engine --check` clean. Probe `/tmp/arr.js`: all 10 forms (`slice`, `splice`, `Array.of`, `Array.from`, `flat`, `Array(3)`, `new Array(3)`, `"a,b".split(",")`, `"abc".match(/./g)`, `"abc".split(/(b)/)`) report `Array.isArray === true`, `constructor === Array`, `Object.getPrototypeOf(x) === Array.prototype`; `arr.foo = 42` reads back `42`; an added `Array.prototype.someMethod` is reachable.
- **Remaining gap (out of scope):** `Array.prototype.map`/`filter`/`forEach`/`reduce`/`flatMap`/`slice`-callback results are allocated at the interpreter's callback-builtin seam (`crates/v12-interp/src/call_setup.rs`, ~14 `JsObject::array` sites); that file is outside this lane's write scope, so those results are still unlinked.
...
### 2026-09-18 — lane/import-defer: `import defer` no longer evaluates eagerly (lazy loader slice) [lane/import-defer]

- **Filters:** `language/import/import-defer` (103), `language/import` (129), `language/module-code` (599), `--jobs 8`, plus TAP pass-set diffs. Base master `920fc80`.
- **Before:** import-defer **12 pass / 91 fail**; `language/import` **19 pass / 110 fail (14.7 %)**; module-code **437 / 162 / 0 (73.0 %)**.
- **After:** import-defer **50 / 53**; `language/import` **57 / 72 (44.2 %)**; module-code **437 / 162 (73.0 %)** — unchanged. Net **+38** on `language/import`, **zero** lost passes (TAP pass-set diff `comm -23` empty on both filters).
- **Root cause:** the compiler read the import declaration but ignored `ImportDeclaration::phase`, so `ImportEntry` carried no phase; both static-graph drivers (`module_loader::load_and_evaluate` *and* the entry-module loop in `engine/eval.rs`) recursed into every dependency unconditionally. A deferred dependency therefore ran its body before the importer, so `evaluations.length` was already 1 at the first `assert.sameValue(…, 0)` (dominant message `import defer does not trigger evaluation`, 71 tests).
- **Fix:** (1) `collect.rs` captures `Some(ImportPhase::Defer)` into a new `ImportEntry.deferred` flag on all five construction sites; (2) `module_loader.rs` gains a `LoaderState.deferred` map and `load_deferred` — compile + namespace + `seed_export_keys` but no body execution — plus `read_and_compile` factored out of `load_and_evaluate`; both dependency loops (`load_and_evaluate` and the `engine/eval.rs` entry loop) dispatch deferred requests to `load_deferred`; `handle_import` resolves a deferred namespace from the new map. (3) Spec correction `GatherAsynchronousTransitiveDependencies`: a deferred module (or transitively reached dependency) with top-level await must still evaluate eagerly, so `gather_async_targets` collects those and `load_deferred` runs them; without it, `evaluation-top-level-await/import-defer-async-module` regressed (it was the only lost pass in the first cut).
- **Files:** `crates/v12-bccompiler/src/collect.rs` (+13/−2), `crates/v12-bccompiler/src/model.rs` (+6), `crates/v12-engine/src/module_loader.rs` (+146/−9), `crates/v12-engine/src/engine/eval.rs` (+9/−4, the ~2-line entry-loop change authorised by the task owner).
- **Laziness proof (`evaluation-triggers/ignore-not-exported-then-get.js`):** before `threw: import defer does not trigger evaluation Expected SameValue(«1», «0») to be true`; after `ok`. Same for `ignore-exported-then-get.js` and `ignore-symbol-toStringTag-get.js`. Node v24.21.0 does **not** ship `import defer` (SyntaxError `Unexpected token '*'`), so the executable reference is the test262 spec test, not a node-vs-v12 run.
- **Remaining (this lane, not landed):** the deferred-namespace `[[Get]]` trigger. Reading an export must evaluate the deferred module exactly once, *before* the get completes; the trigger lives in `v12-interp::get_property` (explicitly out of this lane's ownership), and no owned seam can re-enter the interpreter from the engine — so the `trigger-*` and `errors/*` buckets (~40 tests) still fail. `@@toStringTag` ("Deferred Module") on deferred namespaces and `import.defer()` are likewise unlanded.
- **Verification:** `cargo build --workspace` clean; `cargo nextest run --workspace` **640 passed / 0 skipped**; `cargo clippy --workspace --all-targets` **0 errors**; `cargo fmt -p v12-bccompiler -p v12-engine` then `cargo fmt --check` clean.
...
### 2026-09-18 — [lane/proxy-traps] Proxy internal methods route through handler traps (ES 10.5)

- **Filters:** `built-ins/Proxy` (311), `language/expressions` (11 128), `language/expressions/object` (1 170), `--jobs 8`, default `--format human`; per-test TAP pass-set diffs at each checkpoint.
- **Before (preserved WIP `15a972e`, built clean):** `built-ins/Proxy` **229 pass / 81 fail / 1 skip (73.9 %)**. Dominant buckets: `set` 16, `has` 12, `construct` 10, `defineProperty` 7, `getOwnPropertyDescriptor` 6, `get` 6, `revocable` 4, `ownKeys` 4, `apply` 3, `deleteProperty` 3.
- **After:** `built-ins/Proxy` **273 pass / 37 fail / 1 skip (88.1 %, +44)**. `language/expressions` **7 738 → 7 742 pass / 3 370 fail / 16 skip (69.7 %)**; `language/expressions/object` **850 / 320 / 0 (72.6 %)**. Zero previously-passing tests lost on any filter (TAP pass-set diffs against every prior checkpoint, including the original baseline).
- **Withheld WIP blocker (fixed first, commit `5081c59`):** the preserved `15a972e` changed `prepare_construct`'s non-object-callee message from `"value is not a constructor"` to `"callee is not a function"`, breaking `tests::construct_non_constructor_throws_type_error`. The message is the *call* path's; restored the construct wording. Do not edit the test.
- **Fixes (one commit each, all in `crates/v12-interp/src/{property,call_setup,execute}.rs`):**
  - `780b067` prototype walks treated a `Kind::Proxy` link as an ordinary object: `op_in`/`ic_lookup` now dispatch the proxy ancestor's `has`/`get` trap (the `get` forward keeps the *original* receiver, ES 10.5.8). `Object.getOwnPropertyNames`/`getOwnPropertySymbols` now use the `GetOwnPropertyKeys` path (filter `[[OwnPropertyKeys]]` by key type) instead of consulting `[[GetOwnProperty]]`, so an `ownKeys`-only handler no longer fires the descriptor trap.
  - `f5cb43b` `Reflect.apply`/`Reflect.construct` were registry natives that rejected proxy and bytecode targets ("requires the interpreter"). Moved to the interp callback seam: `CreateListFromArrayLike` runs through the interpreter, a proxy target routes through `[[Call]]`/`[[Construct]]` (recursing through nested proxies), and Native/Host constructor targets are invoked directly.
  - `e4b0ca7` `[[Set]]` gained its boolean channel (falsy trap result → strict TypeError, `Reflect.set` returns the boolean), receiver forwarding (ES 10.1.9 `OrdinarySetWithOwnDescriptor` via new `ordinary_set_with_receiver`/`receiver_store`), and the steps 12-16 target invariants. `set_property`'s prototype walk now consults a proxy ancestor's `[[Set]]` per level.
  - `c776a79` `define_own_data_attrs` appended a *duplicate* descriptor for an existing shape-backed key, so `find(key)` returned the stale slot and every redefine was invisible — it now updates in place via `update_data_attrs`. Added the `has` steps 8-10 and `get` steps 11-15 invariants, `ordinary_get_with_receiver`, and corrected `is_constructor_object` (Native/Host functions are not constructors unless a registered constructor-shaped id; a Bound target recurses).
  - `c1331a6` `FromPropertyDescriptor` defaulted absent fields to `false`, so a partial `{value}` descriptor made a target property non-writable on the first proxy write; absent fields now stay absent. `receiver_store`'s absent-property branch now uses the full all-true `CreateDataProperty` descriptor (ES 10.1.9 step 2.e).
  - `ba8714c` strict-mode `delete` threw nothing when `[[Delete]]` returned false (ES 13.5.1.2).
  - `f8922e3` `desc_compatible` re-verified a non-configurable current cannot become configurable (ES ValidateAndApplyPropertyDescriptor step 8).
  - `bf84e45` `OrdinaryHasInstance` read the raw `prototype` field; it now walks `[[GetPrototypeOf]]` so a proxy link dispatches its trap and its non-extensible mismatch TypeError fires.
  - `92df342` corrected `proxy_op_get_own_property` steps 16-18 (the writable-mismatch check was inverted; a non-configurable trap result now requires an existing non-configurable target descriptor).
  - `f9e07c1` a write to a *hole* array index wrote the element store directly (skipping the prototype walk); holes now walk the chain so an `Array.prototype` setter or proxy `set` trap is observed.
- **Probes:** non-constructor `TypeError` (`new 5` → "value is not a constructor"); trap `this === handler` for all of get/set/has/ownKeys/gOPD/apply/construct; nested-proxy forwarding for get/set/has/ownKeys/gOPD/delete/define/getPrototypeOf/preventExtensions/setPrototypeOf; `Reflect.apply`/`construct` over bytecode, proxy, nested-proxy, bound, and native (`Array`) targets; partial-descriptor redefinition repeated three times.
- **Files:** `crates/v12-interp/src/call_setup.rs`, `crates/v12-interp/src/property.rs`, `crates/v12-interp/src/execute.rs`. No other source file changed.
- **Verification:** `cargo build --workspace` clean; `cargo nextest run --workspace` **640 passed / 0 skipped** (green at every commit and at the blocker fix); `cargo clippy --workspace --all-targets` 0 errors; `cargo fmt -p v12-interp -p v12-engine` then `cargo fmt --check` clean.
- **Commits:** `5081c59`, `780b067`, `f5cb43b`, `e4b0ca7`, `c776a79`, `c1331a6`, `ba8714c`, `f8922e3`, `bf84e45`, `92df342`, `f9e07c1` (+ two `cargo fmt` commits `e4e8c53`, `93bd33b`) on `lane/proxy-traps`.
- **Remaining (37 fail):** primitive-backed proxy targets — `new Proxy(new String(...)/RegExp/…)` throws "Cannot create proxy with a non-object" because primitives are not objects (7: get/has/gOPD/ownKeys/deleteProperty `trap-is-null-target-is-proxy`, `trap-is-missing-target-is-proxy`); `%Proxy%.prototype`/`revoke.prototype` are not linked to `Function.prototype`, and the revocation function is not stamped non-constructor (6: `function-prototype`, `revocable/builtin`, `revocable/revocation-function-prototype`, `revocable/not-a-constructor`, `create-target-is-revoked-function-proxy`, `revocable/target-is-revoked-function-proxy`); `%Proxy%` is a shape-bound global, not an intrinsic own property (`proxy.js`); cross-realm `$262.createRealm().global.eval(new Proxy(...))` and `Object.getPrototypeOf(desc)` realm linking (5: `apply`/`construct` `arguments-realm`, `defineProperty/desc-realm`, construct proto-from-cross-realm-newtarget); bound-function construct (`get-fn-realm*`, `construct/trap-is-undefined-target-is-proxy`); `with`-statement write resolution, `for-in` binding patterns, and the remaining `has`/`set`/`getOwnPropertyDescriptor`/`preventExtensions` `trap-is-*-target-is-proxy` nested cases.
